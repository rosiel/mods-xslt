...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="mods:mods/mods:titleInfo[@type='alternative' or @type='uniform' or @type='abbreviated']">
        <field_alt_title>
            <xsl:call-template name="titleInfoFormatter"></xsl:call-template>
        </field_alt_title>
    </xsl:template>
    ...
    <!-- MODS name -->
    <xsl:template match="mods:mods/mods:name[(@type='personal' or @type='corporate' or @type='family') and mods:namePart/text()]">
        <xsl:if test="count(mods:namePart/text())>0">
            ...
            <xsl:choose>
                <xsl:when test="mods:role/mods:roleTerm[@authority='marcrelator' and @type='code']">
                    <xsl:value-of select="mods:role/mods:roleTerm[@authority='marcrelator' and @type='code']/text()"/>
                    <xsl:text>:</xsl:text>
                </xsl:when>
                <xsl:when test="mods:role/mods:roleTerm[@authority='marcrelator' and @type='text']">
                    <xsl:variable name="roleText" select="mods:role/mods:roleTerm[@authority='marcrelator' and @type='text']/text()"/>
                    <xsl:variable name="roleText" select="concat(upper-case(substring($roleText,1,1)),substring($roleText,2))"/>
                    <xsl:variable name="roleCode" select="document('relators.xml')/relators/relator[text()=$roleText]/@code"/>
                    <xsl:choose>
            ...
            <xsl:if test="$nameType='corporate'">
                <xsl:variable name="nameType" select="replace($nameType, 'corporate', 'corporate_body')"/>
    ...
    <xsl:template match="mods:mods/mods:typeOfResource">
        <field_resource_type>
            ...
            <xsl:variable name="resourceType" select="concat(upper-case(substring($resourceType,1,1)),substring($resourceType,2))"></xsl:variable>
            <xsl:value-of select="$resourceType"/>
        </field_resource_type><xsl:comment>TODO: Reconcile resourceType to taxonomy.</xsl:comment>
    ...
    <!-- OriginInfo -->
    <xsl:template match="mods:mods/mods:originInfo/mods:place/mods:placeTerm[@authority='marccountry' and @type='code']">
        <field_place_published_country>
            <xsl:value-of select="normalize-space(.)"/>
        </field_place_published_country>
    </xsl:template>
    <xsl:template match="mods:mods/mods:originInfo/mods:place/mods:placeTerm[not(@type) or @type='text']">
        ...
        <field_language>
            <xsl:choose>
                ...
                <xsl:when test="mods:languageTerm[@authority='iso639-2b' and @type='code']">
                    <xsl:variable name="langCode" select="mods:languageTerm[@authority='iso639-2b' and @type='code']/text()"/>
                    <xsl:variable name="langText" select="document('ISO-639-2.xml')/languages/language[@code=$langCode]/text()"/>
                    <xsl:choose>
        ...
        <field_note>
            <xsl:choose>
                <xsl:when test="@displayLabel">
                    ...
                    <xsl:value-of select="concat($notePrefix, text())"/>
                </xsl:when>
                <xsl:when test="@type">
                    <xsl:variable name="notePrefix" select="concat(upper-case(substring(@type, 1, 1)),substring(@type,2), ': ')"/>
                    <xsl:value-of select="concat($notePrefix, text())"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="text()"/>
    ...
    <xsl:template name="name">
        <xsl:choose>
            ...
            <xsl:when test="count(mods:namePart[not(@type)])=0">
                ...
                <xsl:variable name="termsOfAddress" select="mods:namePart[@type='termsOfAddress']/text()"/>
    ...
    <xsl:template name="subject">
        ...
        <xsl:for-each select="mods:cartographics/coordinates">
            <field_coordinates>
                <xsl:value-of select="."/>
            </field_coordinates>
        </xsl:for-each>
